...
import { capturePointer, releasePointer } from "@/renderer/lib/touch";
  ...
  const onKeyDown = useCallback(
    (event: KeyboardEvent<HTMLElement>) => {
      ...
      if (event.key in map) {
        ...
        const next = snapSliderValue(map[event.key], lo, hi, stride);
        commit(next);
        onValueCommit?.(next);
      }
    },
    [disabled, current, stride, lo, hi, commit, onValueCommit],
  );
