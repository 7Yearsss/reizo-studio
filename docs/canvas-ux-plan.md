...
已落地（`feat/helios-borrowings-canvas`）：`studio` P0-3 运镜（`b4b3d04`）、
P0-1 动作条（`c4b0278`）、P0-2 变体宫格（`0ec6bda`）、P1-1 参考图钉（`18f7471`）；
§8 Runway 全套 —— RW-1/RW-2 底部导航条 + 左竖条 + 框选（`0400a17`）、
RW-4 能量流动边 + 两段式剪线、RW-3 强类型句柄（`NodeHandle`）。
（UX-1 顶部工具栏 `d2dacc4` 已被 RW-1 取代。）
剩余：UX-3（节点解剖图归位）、UX-6（Agent 活动薄条）、UX-7（对话动词收敛）、
UX-8（右面板），以及 `studio` P1-2（llm 分镜编剧节点，Runway 转向后暂缓）。
...
| RW-1 | 底部导航条 + 框选 / 选中缩放（§8） | `0400a17`，取代 UX-1 | ✅ 已合 |
| RW-2 | 左侧竖条（创建 / 整理 / 运行 / 更多）（§8） | `0400a17` | ✅ 已合 |
| RW-3 | 强类型句柄：塌缩/展开 + 同类型同色 + 渐进 `ref_N` 槽 + `*` | `NodeHandle` + `edgeStyles` + 执行器排序 | ✅ 已合 |
| RW-4 | 能量流动边 + 两段式剪线（§8） | `CuttableEdge` 重写 + `index.css` | ✅ 已合 |
